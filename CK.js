const { KeyGen512, Encrypt512, Decrypt512 } = require('./kyber512.js'); // Adjust path as needed
...
(async function () {
  try {
    console.log("Generating Kyber512 key pair...");
    const [publicKey, privateKey] = KeyGen512();

    console.log("Public Key:", publicKey);
    console.log("Private Key:", privateKey);

    console.log("Encrypting shared secret...");
    const [ciphertext, sharedSecret] = Encrypt512(publicKey);

    console.log("Ciphertext:", ciphertext);
    console.log("Shared Secret (Sender):", sharedSecret);

    console.log("Decrypting shared secret...");
    const recoveredSecret = Decrypt512(ciphertext, privateKey);

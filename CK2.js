const { KeyGen1024, Encrypt1024, Decrypt1024 } = require('./kyber1024.js'); // Adjust path as needed
...
(async function () {
  try {
    ...
    const [publicKey, privateKey] = KeyGen1024();

    console.log("Public Key:", publicKey);
    console.log("Private Key:", privateKey);

    console.log("Encrypting shared secret...");
    const [ciphertext, sharedSecret] = Encrypt1024(publicKey);

    console.log("Ciphertext:", ciphertext);
    console.log("Shared Secret (Sender):", sharedSecret);

    console.log("Decrypting shared secret...");
    const recoveredSecret = Decrypt1024(ciphertext, privateKey);
